  ...
  { // Loop until no more g-code words in line.
    ...
    case 'M':
      ...
      case 63:
        ...
        {
        case 62:
          gc_block.modal.output_sync = OUTPUT_SYNC_ON;
          break;
        case 63:
          gc_block.modal.output_sync = OUTPUT_SYNC_OFF;
          break;
        }
        break;
#endif
...
// [9. Override control ]: Not supported except for a Grbl-only parking motion override control.
  ...
  */
  ...
  if (axis_command == AXIS_COMMAND_MOTION_MODE)
  {
    if (gc_block.values.l == 1) // L0
      current_realtime_output_pin_status = 0;
    else if (gc_block.values.l == 2) // L1
      current_realtime_output_pin_status = 1;
