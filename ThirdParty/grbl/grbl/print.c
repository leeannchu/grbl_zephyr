...
__weak char pgm_read_byte_near(const char *s);
...
/**
 * This function is used to read a byte from the flash memory for AVR.
 * In the case of the STM32F7XX, the flash memory is not directly accessible
 */
__weak char pgm_read_byte_near(const char *s)
{
  return *s;
