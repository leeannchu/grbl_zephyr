  ...
  if (parameter >= AXIS_SETTINGS_START_VAL)
  {
    ...
    while (set_idx < AXIS_N_SETTINGS)
    {
      if (parameter < N_AXIS)
      {
        ...
        case 4:
          settings.mm_per_rev[parameter] = value;
          break;
        }
...
#ifdef ZEPHYR_ARCH
    case 33:
      settings.ip_address_0 = int_value;
      break;
    case 34:
      settings.ip_address_1 = int_value;
      break;
    case 35:
      settings.ip_address_2 = int_value;
      break;
    case 36:
      settings.ip_address_3 = int_value;
      break;
    case 37:
...
IO_TYPE get_step_pin_mask(uint8_t axis_idx)
{
  if (axis_idx == X_AXIS)
  {
    return ((1 << X_STEP_BIT));
...
IO_TYPE get_direction_pin_mask(uint8_t axis_idx)
{
  if (axis_idx == X_AXIS)
  {
...
IO_TYPE get_limit_pin_mask(uint8_t axis_idx)
{
  if (axis_idx == X_AXIS)
  {
