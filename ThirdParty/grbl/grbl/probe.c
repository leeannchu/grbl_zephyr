...
#define PROBE_NODE DT_ALIAS(probe)

static const struct gpio_dt_spec probe_dev = GPIO_DT_SPEC_GET(PROBE_NODE, gpios);
#endif
...
void probe_init()
{
  ...
  flags |= GPIO_NOPULL;
#endif
  gpio_pin_configure_dt(&probe_dev, flags);
#endif                                // AVR_ARCH
...
IO_TYPE probe_get_state()
{
#ifdef ZEPHYR_ARCH
  int pin_state = gpio_pin_get_dt(&probe_dev);
