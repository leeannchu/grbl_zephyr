...
#ifdef ZEPHYR_ARCH
#define X_LIMIT_NODE DT_ALIAS(x_limit)
#define Y_LIMIT_NODE DT_ALIAS(y_limit)
#define Z_LIMIT_NODE DT_ALIAS(z_limit)

static const struct gpio_dt_spec x_limit = GPIO_DT_SPEC_GET(X_LIMIT_NODE, gpios);
static const struct gpio_dt_spec y_limit = GPIO_DT_SPEC_GET(Y_LIMIT_NODE, gpios);
static const struct gpio_dt_spec z_limit = GPIO_DT_SPEC_GET(Z_LIMIT_NODE, gpios);

static const struct gpio_dt_spec *limit_switches[N_AXIS] = {
    &x_limit,
    &y_limit,
    &z_limit};

static struct gpio_callback limit_cb_data[N_AXIS];
...
#ifndef HOMING_AXIS_SEARCH_SCALAR_F
#define HOMING_AXIS_SEARCH_SCALAR_F 1.5f // Must be > 1 to ensure limit switch will be engaged.
#endif
...
void limits_callback(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
  ...
  {
    if (dev == limit_switches[i]->port && (pins & BIT(limit_switches[i]->pin)))
    {
      axis = i;
      break;
    }
...
void limits_init()
{
  ...
  /* Overwrite the code generated by CubeIDE to fullfill the settings of limit pins */
  ...
  for (int i = 0; i < N_AXIS; i++)
  {
    const struct gpio_dt_spec *pin = limit_switches[i];
    ...
    if (bit_istrue(settings.flags, BITFLAG_HARD_LIMIT_ENABLE))
    {
      ...
      {
        interrupt_mode = GPIO_INT_EDGE_TO_ACTIVE;
      }
      else
      {
        interrupt_mode = GPIO_INT_EDGE_TO_INACTIVE;
      }

      ret = gpio_pin_interrupt_configure_dt(pin, interrupt_mode);
      if (ret < 0)
      {
        continue;
      }

      /* 4. Initialize and Add Callback */
      gpio_init_callback(&limit_cb_data[i], limits_callback, BIT(pin->pin));
      ret = gpio_add_callback(pin->port, &limit_cb_data[i]);
      if (ret < 0)
      {
        continue;
      }
    ...
    {
      gpio_pin_interrupt_configure_dt(pin, GPIO_INT_DISABLE);
    }
  }
#endif // ZEPHYR_ARCH
}
...
#elif defined(ZEPHYR_ARCH)
  // disable the interrupt for the limit pins
  for (int i = 0; i < N_AXIS; i++)
  {
    const struct gpio_dt_spec *pin = limit_switches[i];
...
uint8_t limits_get_state()
{
  ...
  for (int i = 0; i < N_AXIS; i++)
  {
    int val = gpio_pin_get_dt(limit_switches[i]);
    if (val > 0)
    {
      limit_state |= (1 << i);
    }
  ...
  IO_TYPE pin = (LIMIT_PIN & LIMIT_MASK);
...
void limits_go_home(uint8_t cycle_mask)
{
  ...
  for (idx = 0; idx < N_AXIS; idx++)
  {
    ...
    if (bit_istrue(cycle_mask, bit(idx)))
    {
      ...
      max_travel = max(max_travel, (-HOMING_AXIS_SEARCH_SCALAR_F) * settings.max_travel[idx]);
    }
  }
#ifdef ENABLE_DUAL_AXIS
  ...
  do
  {
    ...
    //__enable_irq();
    irq_unlock(key);
    k_msleep(settings.homing_debounce_delay); // Delay to allow transient dynamics to dissipate.
#endif                                        // ZEPHYR_ARCH
